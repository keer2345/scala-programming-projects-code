...
class ReturnsSpec extends AnyWordSpec with Matchers with TypeCheckedTripleEquals {
  ...
  "Returns" when {
    "fromEquityAndInflationData" should {
      "compute real total returns from equity and inflation data" in {
        ...
        val inflations =
          Vector(InflationData("2117.01", 100.0), InflationData("2117.02", 102.0), InflationData("2117.03", 102.0))
  ...
  "Returns.object" when {
    "monthlyRate" should {
      ...
      val variableReturns =
        VariableReturns(Vector(VariableReturn("2000.01", 0.1), VariableReturn("2000.02", 0.2)))
